    ...
    <div data-cmp="TryOnResult" className="w-full">
      ...
      >
        ...
        <div className="absolute inset-0">
          {originalUrl ? (
            <img src={originalUrl} alt="原始照片" className="w-full h-full object-cover" />
          ) : (
        ...
        >
          {originalUrl ? (
            <div className="relative w-full h-full">
              <img src={originalUrl} alt="试穿效果" className="w-full h-full object-cover" />
              ...
              <div
                className="absolute inset-0"
                style={{
                  background: "radial-gradient(ellipse 60% 40% at 50% 35%, rgba(196,168,122,0.25) 0%, transparent 70%)",
                }}
              />
            </div>
          ) : (
          ...
          <div className="absolute bottom-3 right-3 px-2 py-1 rounded-md bg-primary/90 backdrop-blur-sm">
            <span className="text-xs font-medium text-primary-foreground">AI 试穿 · {clothingName}</span>
          </div>
        </div>
        ...
        >
          ...
          {showHint && (
            ...
            >
              <div
                className="mb-2 px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-card/90 backdrop-blur-sm text-foreground shadow-custom"
              >
                左右拖动对比
              </div>
      ...
      <div className="flex gap-3 mt-4">
        <div className="flex-1 rounded-xl overflow-hidden border border-border" style={{ aspectRatio: "2/3" }}>
          {originalUrl ? (
        ...
        <div className="flex-1 rounded-xl overflow-hidden border border-primary" style={{ aspectRatio: "2/3" }}>
          {originalUrl ? (
            <div className="relative w-full h-full">
              <img src={originalUrl} alt="试穿效果" className="w-full h-full object-cover" />
              <div className="absolute inset-0" style={{ background: RESULT_OVERLAYS[overlayIndex] }} />
            </div>
          ) : (
            <div className="w-full h-full bg-secondary flex items-center justify-center">
              <span className="text-2xl">✨</span>
      ...
      <div className="mt-5 p-4 rounded-xl bg-muted">
        <p className="text-xs font-semibold text-foreground mb-2">对这件服装的试穿效果满意吗？</p>
