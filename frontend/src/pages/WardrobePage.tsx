...
import { Button, Input, Empty, Modal, Form, Input as FormInput, Select } from "@arco-design/web-react";
...
const DEFAULT_CLOTHING = [
  ...
  { id: 2, name: "深绿色西装外套", category: "外套", color: "深绿", season: "秋冬", imageIndex: 1 },
...
const WardrobePage = ({ onNavigate = () => {} }: WardrobePageProps) => {
  ...
  const filteredClothes = clothing.filter((item) => {
    ...
    const matchSeason = activeSeason === "全部季节" || item.season === activeSeason || item.season === "四季";
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              共 <span className="text-forest font-semibold">{clothing.length}</span> 件衣物，
              智能分类管理你的全部单品
            </p>
          </div>
              ...
              {CATEGORIES.map((cat) => (
                ...
                >
                  ...
                  <span className={`text-xs rounded-full px-2 py-0.5 ${
                    activeCategory === cat ? "bg-forest text-primary-foreground" : "bg-muted text-muted-foreground"
                  }`}>
              ...
              <div className="flex items-center gap-1 bg-muted rounded-xl p-1">
                <button
                  ...
                  className={`p-2 rounded-lg transition-all duration-200 ${
                    viewMode === "grid" ? "bg-card shadow-card text-forest" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all duration-200 ${
                    viewMode === "list" ? "bg-card shadow-card text-forest" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
            ...
            {filteredClothes.length > 0 ? (
              <div className="flex flex-wrap gap-4">
                ...
                >
                  ...
                  <span className="text-sm text-muted-foreground group-hover:text-forest mt-2 transition-colors">添加新衣物</span>
                </div>
              </div>
            ) : (
        ...
        {showUpload && (
          ...
          >
            ...
            >
              ...
              <p className="text-muted-foreground text-sm mb-6">上传照片后，AI 将自动识别衣物类型和颜色</p>
              ...
              <div className="border-2 border-dashed border-border rounded-xl p-10 text-center cursor-pointer hover:border-primary hover:bg-secondary transition-all duration-200 mb-6">
                ...
                <p className="text-xs text-muted-foreground mt-1">支持 JPG、PNG、HEIC 格式</p>
              </div>
